...
#Some of the preprocessing and generation was done according to https://github.com/keras-team/keras/blob/master/examples/lstm_text_generation.py
...
from helper_files.preprocessing import pre
...
data = data[:200]
if len(sys.argv) != 7:
    raise SyntaxError('Usage: python generate_augmented.py [name of model] [number of episodes to train] [Saved_model or None] [description] [seq_length][salt and pepper]')
...
if num_epochs >0:
    ...
    sequences, next_base = pre(data,model_type, seq_length, salt_pepper)
...
# Callbacks
    ...
    model.model.fit(inputs, targets,validation_split = 0.1,
              batch_size = batch_size,
              epochs=num_epochs, verbose = 0,
              callbacks=[save_callback, tensorboard_callback,early_stopping])


print('end training, begin generation')
